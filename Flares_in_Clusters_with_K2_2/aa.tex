...
% 5 {} token are mandatory
  ...
  % methods heading (mandatory)
   {Drawing from the complete K2 archive, we searched \input{values/total_number_of_lcs.txt}\unskip$\sim80$~day long light curves of \input{values/total_number_of_stars.txt}open cluster members for flares using the open-source software packages K2SC to remove instrumental and astrophysical variability from K2 light curves, and AltaiPony to search and characterize the flare candidates.}
...
\\ 
\citet{lurie2015} analysed the flares on GJ 1245 A and B, two M5 dwarfs in a 50 Myr old triple system with another M8 dwarf, that were observed in 1 minute cadence during the original Kepler mission. GJ 1245 A and B both fell roughly into the $3000-3250\,$K bin. The authors calculated $FA\approx 10^{-4}$ for both components, similar to the $FA$ value in the 135 Myr old Pleiades in this $T_\mathrm{eff}$.%From their FFDs and fitted values for $\alpha$ for each component we inferred $\beta_\mathrm{s,A}\approx 1900\,\mathrm{yr}^{-1}$  and $\beta_\mathrm{s,B}\approx 2200\,\mathrm{yr}^{-1}$, consistent with $\beta_\mathrm{s}$ for the Pleiades, or Hyades/Praesepe within uncertainties.
%-------------------------------------------
\begin{figure}[ht!]
    \centering
...
%--------------------------------------------------------------------
   \begin{figure*}[t!]
   \centering
            \includegraphics[width=.8\hsize]{pics/rotation/SpT_wise_rotation.png}
         \caption{Rotation histograms for the cluster members that were searched for flares and had measured rotation periods $P_\mathrm{rot}$. Rotation periods were taken from~\citet{douglas2019} for the Hyades (orange solid) and Praesepe (yellow dashed), and from~\citet{rebull_pleiadesrot_2016} for the Pleiades (maroon dotted). Rotation periods were available for $\input{values/min_stars_with_rotation_periods.txt}-\input{values/max_stars_with_rotation_periods.txt}\%$ of the stars that we searched for flares. The percentages in the legends in each panel denote the fraction of our sample that is shown for each cluster.}
