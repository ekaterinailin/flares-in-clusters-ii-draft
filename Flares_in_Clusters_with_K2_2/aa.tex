...
We studied the flaring activity in five open clusters, and found a total of X flares on Y stars. We found most of our flares in the Pleiades and Praesepe, several hundred in the Hyades, a handful in Rup 147, and only one flare candidate we could not exclude in M67.

We used multiple cluster membership studies to select for high-probability memeber, and drew from a host of multiband photometry catalogs to determine $T_\mathrm{eff}$ and stellar luminosities. 

We searched a total of Z K2 long cadence light curves for flares using K2SC to remove rotational variability and instrumental effects, and AltaiPony to automatically detect and determine the properties of flare candidates. We vetted all flare candidates manually and discussed various sources of false positives and incompleteness on both the high and the low energy end of the FFDs.

We investigated how flaring activity unfolded for different age and $T_\mathrm{eff}$ ranges, and found flaring activity as measured by the power law intercept $\beta_\mathrm{s}$ in $ED$ space to decrease from ZAMS to solar age, and from mid M dwarfs to G stars, except for stars later than M5, where activity increased from ZAMS to 690\,Myr.
...
We noticed that our results showed lower activity than predicted by a gyrochronological FFD parametrization with mass and age~\citep{davenport2019} within our temperature bins, and that the model rarely matched the differential decline in age in each  $T_\mathrm{eff}$ bin.
...
Recent flare studies from the first year of operations of the Transiting Exoplanet Survey Satellite  
