...
%LINEBREAK FOR APPENDIX END
   ...
   \title{Flares in open clusters with K2.\thanks{Tables \ref{tab:flares} and \ref{tab:powerlawtable_spt}, and a table of properties derived for the 2111 stars in this study are only available in electronic form
...
\begin{figure}
    ...
    \caption{Flaring activity measure $\beta_\mathrm{100s}$ for the FFDs in Fig.~\ref{fig:powerlawfits_s} as a function of age. The values are grouped by $T_\mathrm{eff}$ (different colors and symbols in the legend), and presented on a log-log scale. $\beta_\mathrm{100s}$ indicates the occurrence rate of flares with $ED>100$ s~(see Eq. \ref{eqn:cumdist}). It is controlled for stellar luminosity, so that we can compare flaring activity across both age, and $T_\mathrm{eff}$, at the same time. The age labels correspond to the open cluster ages in Table \ref{tab:data_clusters}.}    	
