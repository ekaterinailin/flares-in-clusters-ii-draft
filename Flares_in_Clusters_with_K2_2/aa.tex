...
\label{eqn:cumdist}
\end{equation}
We used and compared two approaches to fitting $\alpha$ and $\beta$ to the data. The first was a Modified Maximum Likelihood Estimator (MMLE) for power law distributions~\citep{maschberger_powerlaw_2009}. The second approach was more specifically tailored to flaring activity. We used the MCMC method to sample from the predictive distribution for a flare source that produced flares that followed a Poisson process in time, and are power law distributed in the ensemble \citep{wheatland_flaresbayes_2004}. While the MMLE was computationally efficient, only the predictive distribution allowed us to fit for $\alpha$ and $\beta$ simultaneously.
\paragraph{Modified Maximum Likelihood Estimatator}
We fitted power law functions to the FFDs in $ED$ and $E_\mathrm{Kp}$ space using a stabilized maximum likelihood estimator (MLE)~ \citep{maschberger_powerlaw_2009}. The logarithm of the likelihood function $\mathstt{L}$ that had to be maximized was given in Eqn. (8) of \citet{maschberger_powerlaw_2009}:
\begin{equation}
\log \mathstt{L} = n \log (1-\hat{\alpha})-n \log\left(x_\mathrm{max}^{1-\hat{\alpha}}-x_\mathrm{min}^{1-\hat{\alpha}}\right) - \hat{\alpha} \displaystyle\sum_{i=1}^{n}\log x_i
\label{eqn:MLE}
\end{equation}
where $x_i$, $x_\mathrm{max}$, and $x_\mathrm{min}$ were the detected flare energies, and the upper and lower limits for detection, respectively. $n$ was the total number of flares. The estimate for $\alpha$ was be biased in practice because $x_\mathrm{max}$ was the maximum energy that was measured, and not the underlying upper limit. The stabilization transformation suggested by the authors was then applied to the solution for $\alpha$ to account for this bias:
\begin{equation}
...
\paragraph{Predictive flaring model}
In the second approach we adopted the model descibed in \citet{wheatland_flaresbayes_2004}. Following their derivation, we defined the joint posterior distribution for the probability $\epsilon$ that a flare with $ED$ or $E_\mathrm{Kp}$ above some value $S_2$ would occur within a time period $\Delta T$:
\begin{eqnarray}
